...
class SettingsFrame(ttk.LabelFrame):
    ...
    def setup_ui(self):
        ...
        # Encoder
        ttk.Label(row1, text="Encoder:", font=("Arial", 8)).pack(side=tk.LEFT)
        ...
        self.encoder_combo.pack(side=tk.LEFT, padx=(3, 10))
        self.encoder_combo.bind('<<ComboboxSelected>>', self.on_encoder_change)
        
        # Preset
        ttk.Label(row1, text="Preset:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.preset_var = tk.StringVar()
        self.preset_combo = ttk.Combobox(row1, textvariable=self.preset_var, 
                                        values=PRESETS, state="readonly", width=10)
        self.preset_combo.pack(side=tk.LEFT, padx=(3, 0))
        self.preset_combo.bind('<<ComboboxSelected>>', self.on_preset_change)
        
        # Warnung für Hardware-Encoder
        self.warning_label = ttk.Label(row1, text="", foreground="orange", font=("Arial", 8))
        self.warning_label.pack(side=tk.LEFT, padx=(5, 0))
        ...
        # CRF
        ttk.Label(row2, text="CRF:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.crf_var = tk.IntVar()
        self.crf_scale = ttk.Scale(row2, from_=0, to=51, variable=self.crf_var, 
                                  orient=tk.HORIZONTAL, length=100)
        self.crf_scale.pack(side=tk.LEFT, padx=(3, 5))
        self.crf_label = ttk.Label(row2, text="23", font=("Arial", 8))
        self.crf_label.pack(side=tk.LEFT)
        self.crf_scale.config(command=self.update_crf_label)
        
        # Profile
        ttk.Label(row2, text="Profil:", font=("Arial", 8)).pack(side=tk.LEFT, padx=(10, 0))
        self.profile_var = tk.StringVar()
        self.profile_combo = ttk.Combobox(row2, textvariable=self.profile_var, 
                                         values=ENCODING_PROFILES, state="readonly", width=10)
        self.profile_combo.pack(side=tk.LEFT, padx=(3, 0))
        self.profile_combo.bind('<<ComboboxSelected>>', self.on_profile_change)
        
        # Info über automatische Profil-Anpassung
        self.profile_info = ttk.Label(row2, text="", foreground="blue", font=("Arial", 7))
        self.profile_info.pack(side=tk.LEFT, padx=(5, 0))
        
        # Dritte Zeile: Ausgabeformat, Threads und Farbtiefe
        row3 = ttk.Frame(self)
        row3.pack(fill=tk.X, pady=1)
        
        # Ausgabeformat
        ttk.Label(row3, text="Ausgabe:", font=("Arial", 8)).pack(side=tk.LEFT)
        ...
        self.output_format_combo.pack(side=tk.LEFT, padx=(3, 10))
        
        # Threads
        ttk.Label(row3, text="Threads:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.threads_var = tk.StringVar()
        self.threads_combo = ttk.Combobox(row3, textvariable=self.threads_var, 
                                         values=THREAD_OPTIONS, state="readonly", width=8)
        self.threads_combo.pack(side=tk.LEFT, padx=(3, 10))
        
        # Farbtiefe-Modus
        ttk.Label(row3, text="Farbtiefe:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.color_depth_var = tk.StringVar()
        self.color_depth_combo = ttk.Combobox(row3, textvariable=self.color_depth_var, 
                                             values=list(COLOR_DEPTH_MODES.keys()), 
                                             state="readonly", width=15)
        self.color_depth_combo.pack(side=tk.LEFT, padx=(3, 0))
        self.color_depth_combo.bind('<<ComboboxSelected>>', self.on_color_depth_change)
        
        # Farbtiefe-Beschreibung
        self.color_depth_desc = ttk.Label(row3, text="", foreground="blue", font=("Arial", 7))
        self.color_depth_desc.pack(side=tk.LEFT, padx=(5, 0))
        
        # Vierte Zeile: Ausgabeverzeichnis
        row4 = ttk.Frame(self)
        row4.pack(fill=tk.X, pady=1)
        
        ttk.Label(row4, text="Ausgabe:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.output_dir_var = tk.StringVar()
        self.output_dir_entry = ttk.Entry(row4, textvariable=self.output_dir_var, width=40)
        self.output_dir_entry.pack(side=tk.LEFT, padx=(3, 3), fill=tk.X, expand=True)
